...
- ed1eb30: Hard-cut live events to one awaited typed subscription, add the deterministic
  `whatsappd/testing` driver, retain conversation-sync metadata, rename the
  credential capability, and remove agent-era exports.
- ed1eb30: Capture inbound image, video, audio and voice-note, document, and sticker bytes
  ...
  state.
- ed1eb30: Name the actual author of every message. `InboundMessage.from` and
  ...
  identity source.
- ed1eb30: Carry every mutation kind the projection computed on the patch (ADR-0030,
...
- ed1eb30: Run a local libSQL database in WAL, so a read no longer stops every writer on
...
- ed1eb30: Persist one text message through the core runtime. `createWhatsAppRuntime()`
...
- ed1eb30: Deliver live and durable frames on separate registrations, and rewrite the
...
- ed1eb30: Answer any number of reads about one account at a single revision.
...
- ed1eb30: Page saved messages and recover revision gaps. A client snapshot is now the
...
- ed1eb30: `fileStore()` recreates its directory on every write instead of once at
...
- ed1eb30: Stabilize runtime teardown, lease renewal, session failure precedence, and

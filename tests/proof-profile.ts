 ...
 *
 *   node --experimental-strip-types tests/proof-profile.ts            # client-read
 *   node --experimental-strip-types tests/proof-profile.ts lifecycle  # another profile
 *
 ...
 *
 * Everything lives under the gitignored `.proof-private/<profile>/`. Nothing
...
try {
  ...
  console.log(`   ${path.relative(path.join(here, ".."), directory)} — rerun to resume, no scan.`);
} finally {

 ...
 * make `pnpm check` go red, which is what `proofs/runners/send-guard-proof.ts` does in a
 ...
 *
 * Each fixture also *returns* its call, so `proofs/tests/send-guard.test.ts` can await

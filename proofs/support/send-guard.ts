 ...
 * cannot be recalled. `docs/runbooks/development/real-account-testing.md` lists the three
 * destinations that are permitted; this module is what enforces that list.
 *
 * This is a module, not a paragraph: an obligation is missed when the correct path costs more to type
 ...
 *    send site. `proofs/tests/send-guard.test.ts` pins that with `@ts-expect-error`
 ...
 * a message reaches strangers. `proofs/tests/send-guard.test.ts` scans this file for
 ...
 * {@link SendRefusedError.allowlistPath} is a `proofs/private/` path: report
 ...
 */
export const DEFAULT_ALLOWLIST_PATH = path.join(here, "..", "private", "send-allowlist.json");
...
export class SendRefusedError extends Error {
  ...
  /** The file the verdict came from. A `proofs/private/` path — keep it out of receipts. */

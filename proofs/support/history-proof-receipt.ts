 ...
 *
 *   node --experimental-strip-types proofs/support/history-proof-receipt.ts <observations.db>
 *
...
function privatePatternHits(value: string): number {
  ...
  if (value.includes("proofs/private")) hits++;
  return hits;
}
...
export function writeProofReceipt(options: {
  ...
  const outDir = path.join(cwd, "proofs", "receipts");
  ...
  const formatted = execFileSync(
    ...
    ["fmt", "--stdin-filepath=proofs/receipts/receipt.json"],
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const outDir = path.join(root, "proofs", "receipts");
  mkdirSync(outDir, { recursive: true });

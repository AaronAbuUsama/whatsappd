 ...
 *
 * The type-level half of the guard lives in `proofs/support/send-guard-types.ts` and is
 * proven by `proofs/runners/send-guard-proof.ts`, which removes each `@ts-expect-error`
...
test("the guard contains no subject lookup and no fuzzy matching — exact ids only", async () => {
  ...
  const guard = await readFile(path.join(here, "..", "support", "send-guard.ts"), "utf8");
...
test("the default allowlist path is the profiles' own gitignored file", () => {
  ...
  // `proofs/private/`, which `.gitignore:10` keeps out of the repository.
  assert.equal(path.basename(DEFAULT_ALLOWLIST_PATH), "send-allowlist.json");
  assert.equal(path.basename(path.dirname(DEFAULT_ALLOWLIST_PATH)), "private");

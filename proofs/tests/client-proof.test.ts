...
} from "../runners/client-proof.ts";

const here = path.dirname(fileURLToPath(import.meta.url));
const runners = path.join(here, "..", "runners");
...
test("subject composition imports only the agreed public seams", async () => {
  const source = await readFile(path.join(runners, "client-proof.ts"), "utf8");
...
test("every real-profile harness uses the production allowlist authority and guarded send site", async () => {
  ...
  for (const file of await testSources(runners)) {
    if (file.endsWith(".test.ts")) continue;
    const source = await readFile(file, "utf8");
    const code = codeWithoutComments(source);
    if (!code.includes('proofs", "private')) continue;
    if (!/\b(?:createSession|createWhatsAppRuntime)\s*\(/u.test(code)) continue;
    harnesses.push({ file, source });
  }

  assert.deepEqual(
    harnesses.map(({ file }) => path.relative(runners, file)).sort(),
    [
      "client-proof.ts",
      "e2e.ts",
      "history-proof.ts",
  ...
  for (const { file, source } of harnesses) {
    assert.deepEqual(
      ...
      [],
      `${path.relative(runners, file)} bypasses the send guard`,
    );
  }

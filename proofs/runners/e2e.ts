 ...
 * real WhatsApp.
 *
 *   node --experimental-strip-types proofs/runners/e2e.ts <number>
 *     <number> = where to send the suite, e.g. your own number for loopback:
 *                node --experimental-strip-types proofs/runners/e2e.ts 15551234567
 *
...
const session = createSession({
  store: fileStore(path.join(root, "proofs", "private", "e2e")),
  auth: qrAuth(),
  logger: pino({ level: process.env.LOG_LEVEL ?? "silent" }),
});
const send = guardedSender(session);
const target = resolveAllowlistedTarget(to);
...
async function runSuite(): Promise<void> {
  ...
  let textRef = await send.send(target, { text: "e2e: text ✅" });
  console.log("  ✅ text");
  await sleep(1500);

  await step("location", () =>
    send.send(target, { location: { lat: 51.5074, lng: -0.1278, name: "London" } }),
  );
  await step("contacts", () =>
    send.send(target, {
      contacts: {
  ...
  await step("image (buffer)", () =>
    send.send(target, { image: TINY_JPEG, caption: "e2e: image" }),
  );
  await step("quote reply", () =>
    send.send(target, { text: "e2e: quoted reply ↩︎" }, { quote: textRef }),
  );
  await step("react 👍", () => send.send(target, { react: { to: textRef, emoji: "👍" } }));
  await step("edit", () =>
    send.send(target, { edit: { target: textRef, text: "e2e: text (edited) ✏️" } }),
  );

  const doomed = await send.send(target, { text: "e2e: delete me 🗑️" });
  await sleep(1500);
  await step("delete", () => send.send(target, { delete: doomed }));
...
session.subscribe({
  ...
  async message(m) {
    ...
    if (!m.fromMe && m.kind === "text" && m.text.trim().toLowerCase() === "ping") {
      ...
      const inboundTarget = resolveAllowlistedTarget(m.chatId);
      await send.send(inboundTarget, { text: "pong" }, { quote: refOf(m) });
      await session.setTyping(m.chatId, false);
      await send.send(inboundTarget, { react: { to: refOf(m), emoji: "🏓" } });
